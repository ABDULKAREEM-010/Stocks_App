...
/**
 * Minimal type definitions to satisfy local usage in this file.
 * Expand these as needed to match your real data shapes.
 */
interface UserForNewsEmail {
    email: string;
    name?: string;
    // add other user fields if used elsewhere (e.g. preferences)
}

interface MarketNewsArticle {
    // Allow both string and numeric ids since downstream data may use either
    id?: string | number;
    headline?: string;
    summary?: string;
    ...
    url?: string;
    source?: string;
    // extend with additional fields returned by getNews()
}
